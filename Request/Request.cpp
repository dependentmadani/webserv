...
void Request::ft_http_code()
{
    ...
    http_code[508] = std::string("Loop Detected");
}
...
int Request::url_characters_checker()
{
    ...
    size_t find = _path.find("%");
    std::stringstream ss;
    if (find != std::string::npos)
    {
        char arr[3];
        const char *ptr = _path.c_str();
...
void    Request::set_cgi_content_header(std::string content) {
    _cgi_content_header = content;
}