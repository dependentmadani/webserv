...
void parsing::check_location(t_server *serv, std::string str)
{
    ...
    // str = trim(str);
	// serv->loc->index = true;
	for (size_t i = 0; i < serv->loc->location.size() ; i++)
    {
		...
		if (!strncmp(ptr.c_str(), "location", strlen("location")))
    	{
        	...
        	for (size_t i = strlen("location"); i < ptr.size() ; i++)
        	    ss[j++] = ptr[i];
        	ss[j] = '\0';
        	serv->loc->url_location = static_cast<std::string>(ss);
			serv->loc->url_location = trim(serv->loc->url_location);
			size_t found= serv->loc->url_location.find("/");
